...
import DayChallengeList, { DayChallengeListType, DayChallengeObjType } from "../home/DayChallengeList";
...
const DaySearchResultList = ({ searchWord }: SearchResultProps) => {
  ...
  const getSearchAxios = () => {
    ...
    apiAddress = `/day?lastItemId=7&size=${size}`
    // 에러 해결되어야 가능할듯
    ...
    tokenHttp.get(apiAddress)
      .then((res) => {
        ...
        const searchResultList = challengeList
          .filter((chal: DayChallengeObjType) => {
            if (searchWord) { return chal.title?.includes(searchWord) }
            else { return true }
          })
