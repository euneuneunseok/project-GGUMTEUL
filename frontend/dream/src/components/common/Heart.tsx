...
const Heart = (props: HeartProps) => {
  ...
  // 밤 좋아요
  const handleNightLike = () => {
    if (isLike) {
      tokenHttp
        .delete(`/night/dream/${props.dreamCardId}/unlike`)
        .then((res) => {
      ...
      tokenHttp
        .post('/night/dream/like', { dreamCardId: props.dreamCardId })
        .then((res) => {
          if (res.data.status === 400) {
            console.log('===', res.data.data)
          } else if (res.data.status === 200) {
  ...
  const handleDayLike = () => {
    ...
    if (isLike) {
      tokenHttp
        .delete(`/day/challenge/detail/${props.challengeDetailId}/unlike`)
        .then((res) => {
          if (res.data.status === 400) {
            console.log(res.data.data)
          } else if (res.data.status === 200) {
      ...
      console.log(props.challengeDetailId)
      console.log(props)
      tokenHttp
        .post('/day/challenge/detail/like', {
          challengeDetailId: props.challengeDetailId
        })
        .then((res) => {
          if (res.data.status === 400) {
            console.log('===', res.data.data)
        ...
        <HeartWrap>
          ...
          <div
            ...
            className={`heart-container ${
              themeMode.mode === 'night' ? 'night-mode' : 'day-mode'
            }`}
            // 테마 모드에 따라 밤 낮 좋아요 모드
            onClick={
              themeMode.mode === 'night' ? handleNightLike : handleDayLike
            }
          >
            <input
              id="Give-It-An-Id"
          ...
          <Text
            ...
            $black={location.pathname.includes('day')}
          >
            {' '}
            {newLikeCount}{' '}
          </Text>
